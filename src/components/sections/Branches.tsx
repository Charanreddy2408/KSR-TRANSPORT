...
export const BRANCH_LOCATIONS = [
  { name: "Hyderabad (Head Office)", lat: 17.3311, lng: 78.5906, state: "Telangana" },
...
const branchGroups = [
  { state: "Telangana", cities: ["Hyderabad (Head Office)"], color: "bg-[#0B1F3A] text-white", lat: 17.3311, lng: 78.5906, gmaps: true },
  { state: "Andhra Pradesh", cities: ["Vizag", "Vijayawada"], color: "bg-[#F97316] text-white", lat: 17.7392, lng: 83.2247, gmaps: false },
  { state: "Tamil Nadu", cities: ["Chennai"], color: "bg-slate-100 text-slate-900 border border-slate-200", lat: 13.0827, lng: 80.2707, gmaps: false },
  { state: "Karnataka", cities: ["Bangalore", "Ballari"], color: "bg-slate-100 text-slate-900 border border-slate-200", lat: 12.9716, lng: 77.5946, gmaps: false },
]
    ...
    <Section id="branches" className="bg-slate-50">
      ...
      >
        ...
        <p className="text-slate-600 text-sm sm:text-base">
          Hover on a pin for branch details · click Telangana (Head Office) to open in Google Maps.
        </p>
      </motion.div>
        ...
        {branchGroups.map((branch, index) => (
          ...
          >
            {branch.gmaps ? (
              ...
              <div
                className={`rounded-lg sm:rounded-xl p-3 sm:p-5 min-h-[72px] sm:min-h-0 flex items-center gap-2 sm:gap-3 ${branch.color} shadow-md`}
              >
                <MapPin className="w-5 h-5 sm:w-6 sm:h-6 shrink-0 opacity-90" />
                <div className="min-w-0 flex-1">
                  <h3 className="font-bold text-xs sm:text-base truncate">{branch.state}</h3>
